...
`database-reviewer` and/or `security-reviewer` (read-only) in parallel and fold
their findings into this review, rather than re-deriving items 2–4 and 8 below
from scratch — they hold the fuller, independently-maintained version of those
checks. Do the full list yourself for a smaller diff or when those areas aren't
touched.
...
2. **Authorization** — full checklist in `.claude/agents/security-reviewer.md`'s
   Authorization section; at minimum, check every new action/route has an
   explicit `authorize`/`policy_scope` call.
3. **Model/database mismatch** — full checklist in
   `.claude/agents/database-reviewer.md`'s "Model/schema agreement" section.
   `bundle exec database_consistency` is the mechanical check.
4. **Migration safety** — full checklist in `.claude/agents/database-reviewer.md`'s
...
8. **Environment configuration** — full checklist in
   `.claude/agents/security-reviewer.md`'s "Environment config" section.
